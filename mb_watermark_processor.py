...
class WatermarkBase:
    ...
    def _seed_rng(self, input_ids: torch.LongTensor, seeding_scheme: str = None) -> None:
        ...
        if seeding_scheme == "simple_1":
            ...
            self.digit = random.randint(1, len(self.message))
            bit = self.get_current_bit(self.digit)
            self.rng.manual_seed(self.hash_key * prev_token + self.hash_key * bit)
        else:
    ...
    def get_current_bit(self, digit):
        return int(self.message[digit-1])
...
class WatermarkDetector(WatermarkBase):
    ...
    def _compute_z_score(self, observed_count, T):
        # count refers to number of green tokens for each digit, T is total number of tokens
